...
def backprop(nn, X, Y, loss):
    ...
    for i in reversed(range(num_layers)):                           # Iterate over all layers
        if i == num_layers - 1:
            if loss == 'MSE':
                ...
                # delta[i] /= len(Y_hat)
                # Y = np.array([[Y]])
                # derivative = Y_hat.copy()
                # derivative[Y, range(Y.shape[1])] -= 1
                # if (derivative/len(Y_hat) != delta[i]).all():
                #     print(delta[i])
                # delta[i] = derivative/len(Y_hat)
            elif loss == 'Softmax':
                ...
                '''
                # we multiply by Q(s_t,a_t) outside of backprop.
    ...
    for i in range(num_layers):
        if i == 0:
            ...
            grad[i].append(np.dot(delta[i], X.T))  # for some reason, delta[i] is 0
